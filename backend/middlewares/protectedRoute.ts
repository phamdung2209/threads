...
const protectedRoute = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const decoded = jwt.verify(_auth, process.env.JWT_SECRET as string) as { _id: string } | null
        console.log('decoded: ', decoded)

        if (!decoded) return res.json({ error: 'Unauthorized' })
        const user: IUserDocument = await User.findById(decoded?._id).select('-password')
